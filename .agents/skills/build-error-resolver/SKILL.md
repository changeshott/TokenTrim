...
```typescript
// âŒ ERROR: Parameter 'x' implicitly has an 'any' type
function add(x, y) {
  return x + y
}
...
```typescript
// âŒ ERROR: Object is possibly 'undefined'
...
```typescript
// âŒ ERROR: Property 'age' does not exist on type 'User'
interface User {
  name: string
}
...
```typescript
// âŒ ERROR: Cannot find module '@/lib/utils'
...
```typescript
// âŒ ERROR: Type 'string' is not assignable to type 'number'
const age: number = "30"
...
```typescript
// âŒ ERROR: Type 'T' is not assignable to type 'string'
...
```typescript
// âŒ ERROR: React Hook "useState" cannot be called in a function
...
```typescript
// âŒ ERROR: 'await' expressions are only allowed within async functions
function fetchData() {
...
```typescript
// âŒ ERROR: Cannot find module 'react' or its corresponding type declarations
...
```typescript
// âŒ ERROR: Fast Refresh had to perform a full reload
...
```typescript
// âŒ ERROR: React 19 type changes
...
```typescript
// âŒ ERROR: Type 'any' not assignable
...
### Redis Stack Types
```typescript
// âŒ ERROR: Property 'ft' does not exist on type 'RedisClientType'
...
```typescript
// âŒ ERROR: Argument of type 'string' not assignable to 'PublicKey'
...
### DON'T:
âŒ Refactor unrelated code
âŒ Change architecture
âŒ Rename variables/functions (unless causing error)
âŒ Add new features
âŒ Change logic flow (unless fixing error)
âŒ Optimize performance
âŒ Improve code style
...
// âŒ WRONG: Refactor entire file
// - Rename variables
// - Extract functions
// - Change patterns
...
**Initial Errors:** X
**Errors Fixed:** Y
**Build Status:** âœ… PASSING / âŒ FAILING
